...
class Layer:
	...
	def __init__(self,json,container):
		...
		self.i=int(size)
		self.j=int(size)
		self.img = Image.new( 'RGB',(self.i,self.j), "black") # create a new black image
		...
		for i in range(self.img.size[0]):    # for every pixel:
		    for j in range(self.img.size[1]):
		        if(random.random()>density):
		        	color=linint((red_max,green_max,blue_max),(red_min,green_min,blue_min),random.random())
		        	if(random.random()>large):
		        		self.pixels[i,j]=(int(color[0]),int(color[1]),int(color[2]))
		        	else:
		        		self.pixels[i,j]=(int(color[0]),int(color[1]),int(color[2]))
		        		self.pixels[min(self.img.size[0]-1,i+1),j]=(int(color[0]),int(color[1]),int(color[2]))
		        		self.pixels[min(self.img.size[0]-1,i+1),min(self.img.size[1]-1,j+1)]=(int(color[0]),int(color[1]),int(color[2]))
		        		self.pixels[i,min(self.img.size[1]-1,j+1)]=(int(color[0]),int(color[1]),int(color[2]))
	...
	def temp(self):
		...
		for i in range(self.img.size[0]):    # for every pixel:
		    for j in range(self.img.size[1]):
		    	...
		    	self.pixels[i,j]=(		 int(rgb_min[0]+p*(rgb_max[0]-rgb_min[0])) 
		    							,int(rgb_min[1]+p*(rgb_max[1]-rgb_min[1]))
		    							,int(rgb_min[2]+p*(rgb_max[2]-rgb_min[2])))		
			...
			for j in range(self.img.size[1]):
				if b.pixels[i,j]!=(0,0,0):
					self.pixels[i,j]=(	int(self.pixels[i,j][0]*b.alpha+b.pixels[i,j][0]*(1-b.alpha)),
										int(self.pixels[i,j][1]*b.alpha+b.pixels[i,j][1]*(1-b.alpha)),
